    ...
    try {
        ...
        $(element).find('.info > p:nth-of-type(2) > a').each((i, e) => {
            temp_genre.push({
                name: $(e).text(), 
                url: $(e).attr('href'),
                endpoint: $(e).attr('href').replace('https://rajahentai.xyz/', '')
            });
        });
            
        
        const ress = [];
        $(element).find('.player-embed').each((ind, ele) => {

            const temp_res = [];
            $(ele).find('.mirror').each((i, e) => {
            ...
            ress.push([$(ele).find('.data-default').text(), temp_res]);
        ...
        obj.season = {
            ...
            endpoint: $(element).find('.info > p:nth-of-type(3) > a').attr('href').replace('https://rajahentai.xyz/', '')
        };
        ...
        obj.score = 'тнР' + $(element).find('.info > p:nth-of-type(8)').text().split(':')[1].trim();
